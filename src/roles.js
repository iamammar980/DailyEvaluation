...
(function () {
  ...
  var Roles = {
    ...
    resolve: function (cb) {
      ...
      // try the server-side role node when online
      try {
        if (!window.isOfflineMode && window.currentUser && window.db && typeof dbRef === 'function') {
          dbRef('role').once('value').then(function (snap) {
